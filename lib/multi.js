...
function Multi(context, config) {
  ...
  config = config || {};
  config.interrupt = (config.interrupt !== undefined) ? config.interrupt : false;
  config.parallel = (config.parallel !== undefined) ? config.parallel : false;
  config.flush = (config.flush !== undefined) ? config.flush : true;
