...
function Multi(context, config) {
    ...
    if (stack.length > 0) {
      ...
      if (config.parallel) {
        ...
        self.promise.on('finished', function() {
          ...
          callback.apply(context, preparedArgs); // [err, results]
        });
  ...
  function resultsCallback() {
    ...
    if (config.parallel) {
      ...
      // Sequential execution
      if (++counter == stack.length) {
        var errAndResults = setInitialState(true); // Reset state > return
        callback.apply(context, errAndResults); // [err, results]
      } else {
